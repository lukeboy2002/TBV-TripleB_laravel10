<div>
    ...
    <div class="overflow-x-auto relative shadow-md sm:rounded-lg px-4 py-2">
        <div class="flex justify-between items-center space-x-4 pb-4">
            ...
            <div>
                <x-links.btn-primair href="{{ route('admin.sponsors.create') }}" class="px-5 py-2.5 text-sm font-medium">New sponsor</x-links.btn-primair>
            </div>
        </div>
            ...
            <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                ...
                @forelse($sponsors as $sponsor )
                    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                        ...
                        <th scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                            <x-links.primair href="{{ $sponsor->link }}" target="_blank">{{ $sponsor->link }}</x-links.primair>
                        </th>
                        <th scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap dark:text-white">
                            @if( $sponsor->published =='1' )
                        ...
                        <td class="px-6 py-4 flex justify-end space-x-4">
                            <x-links.btn-primair href="{{ route('admin.sponsors.edit' , $sponsor) }}" class="px-2.5 py-2.5 text-xs font-medium"><i class="fa-solid fa-pen-to-square"></i></x-links.btn-primair>
                            <x-buttons.danger type="button" wire:click="deleteId({{ $sponsor->id }})" class="px-2.5 py-2.5 text-xs font-medium"><i class="fa-solid fa-trash-can"></i></x-buttons.danger>
                        </td>
                    </tr>
                @empty
                    <tr class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
