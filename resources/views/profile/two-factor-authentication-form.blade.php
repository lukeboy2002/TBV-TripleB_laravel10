        ...
        <div class="mt-5">
            @if (! $this->enabled)
                <x-sections.confirms-password wire:then="enableTwoFactorAuthentication">
                    <x-buttons.primair type="button" class="px-3 py-2 text-xs font-medium" wire:loading.attr="disabled">
                        Enable
                    </x-buttons.primair>
            ...
            @else
                @if ($showingRecoveryCodes)
                    <x-sections.confirms-password wire:then="regenerateRecoveryCodes">
                        <x-buttons.secondary class="px-3 py-2 text-xs font-medium mr-2">
                            Regenerate Recovery Codes
                        </x-buttons.secondary>
                    </x-sections.confirms-password>
                @elseif ($showingConfirmation)
                    <x-sections.confirms-password wire:then="confirmTwoFactorAuthentication">
                        <x-buttons.primair type="button" class="px-3 py-2 text-xs font-medium mr-2" wire:loading.attr="disabled">
                            Confirm
                        </x-buttons.primair>
                    </x-sections.confirms-password>
                @else
                ...
                @if ($showingConfirmation)
                    <x-sections.confirms-password wire:then="disableTwoFactorAuthentication">
                        <x-buttons.secondary class="px-3 py-2 text-xs font-medium mr-2" wire:loading.attr="disabled">
                            Cancel
                        </x-buttons.secondary>
                    </x-sections.confirms-password>
                @else
                    <x-sections.confirms-password wire:then="disableTwoFactorAuthentication">
                        <x-buttons.danger class="px-3 py-2 text-xs font-medium mr-2" wire:loading.attr="disabled">
                            Disable
                        </x-buttons.danger>
                    </x-sections.confirms-password>
                @endif
